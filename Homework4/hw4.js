...
function reserveDate(cal) {
    ...
    const choice = prompt('Choose a date to reserve (enter the number): ');
    const chosenDate = availableDates[parseInt(choice) - 1];

    if (!chosenDate) {
        throw new Error('Invalid choice. Exiting the program.');
    }

    const attendeeName = prompt('Enter your name for the reservation: ');
    const attendeeEmail = prompt('Enter your email for the reservation: ');
    let startTime = prompt('Enter the start time for the reservation (HH:MM AM/PM): ');
    ...
    const summary = prompt('Enter a summary for the event: ');
...
function cancelReservation(cal) {
    const id = prompt('Enter the event ID to cancel: ');
    const events = cal.events();

    const eventIndex = events.findIndex(event => event.uid() === id);
    if (eventIndex !== -1) {
...
function lookupReservations(cal) {
    const id = prompt('Enter the event ID to lookup: ');
    const event = cal.events().find(event => event.uid() === id);
